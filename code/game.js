...
Unfast.prototype.type = "unfast";

function lowGrav(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
lowGrav.prototype.type = "lowGrav";

function slow(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.1, 0.1));
  this.size = new Vector(0.4, 0.2);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
slow.prototype.type = "slow";

function myLevels(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
myLevels.prototype.type = "myLevels";

function monster(pos, ch) {
  this.pos = pos;
  this.size = new Vector(6, 7);
  if (ch == "<") {
    // Horizontal monster
...
Unfast.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

lowGrav.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
myLevels.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if (keys.left) Player.prototype.type="runLeft";
  if (keys.right) Player.prototype.type="runRight";
...
if (!this.actors.some(function(actor) {
  ...
  if (type == "monster" || type == "lava" && this.status == null) {
    Player.prototype.type = "player";
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
